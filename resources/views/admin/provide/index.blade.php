    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="row mb-2">
                    <div class="col-sm-4">
                        <a href="{{ route('admin.provide.create') }}" class="btn btn-danger mb-2"><i
                                class="mdi mdi-plus-circle mr-2"></i> Add Provide</a>
...
<script>
    $(document).ready(function() {
        ...
        function fetchData(page) {
            $.ajax({
                url: "{{ route('api.provide.index') }}" + `?page=${page}`,
                method: 'GET',
                dataType: 'json',
        ...
        function updateUI(data) {
            ...
            for (let i = 0; i < data.data.length; i++) {
                ...
                $('tbody').append('<tr>')
                    .append(`<td>${i + 1}</td>`)
                    .append(`<td>${v.provider_name}</td>`)
                    .append(`<td>${v.product_name}</td>`)
                    .append(
                        `<td class="table-action">
                            <a href="/admin/provide/edit/${v.provider_id}/${v.product_id}" class="action-icon"> 
                                <i class="mdi mdi-pencil"></i></a>
                            <a href="##" class="action-icon btn-delete" provider-id='${v.provider_id}' product-id='${v.product_id}'> 
        ...
        $('tbody').on('click', '.btn-delete', function(e) {
            ...
            swal({
                ...
                $.ajax({
                    type: "POST",
                    url: `/admin/provide/destroy/${providerId}/${productId}`,
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "_method": 'DELETE'
                    },
